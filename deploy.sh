#!/usr/bin/env bash
# =================================================================
# 项目：Sing-box 自动化管理系统 (V9.8 纯净修正版)
# =================================================================
...
install_singbox() {
    ...
    if [ ! -f "$CERT_DIR/server.crt" ]; then
        echo -e "${YELLOW}正在申请证书 (验证 80 端口)...${NC}"
        ...
        if ! /root/.acme.sh/acme.sh --issue -d "$DOMAIN" --standalone --force; then
            echo -e "${RED}申请失败！请检查云面板 80 入站规则。${NC}"
            exit 1
        fi
        mkdir -p "$CERT_DIR"
...
EOF
    ...
    echo -e "${GREEN}部署完成！请使用选项 3 获取二维码。${NC}"
...
show_config() {
    clear
    [ ! -f "$CONF_DIR/config.json" ] && { echo -e "${RED}未安装服务。${NC}"; sleep 1; main_menu; }
    UUID=$(jq -r '.inbounds[0].users[0].uuid' "$CONF_DIR/config.json")
    PUB_KEY=$(cat "$CONF_DIR/pub.key")
    SID=$(jq -r '.inbounds[0].tls.reality.short_id[0]')
    DOMAIN=$(jq -r '.inbounds[1].tls.server_name')
...
main_menu() {
    clear
    echo -e "${BLUE}==================================================${NC}"
    echo -e "      不正经的科学 - Sing-box 管理系统 V9.8       "
    echo -e "${BLUE}==================================================${NC}"
    echo -e "  ${GREEN}1.${NC} 开启 BBR 优化"
    echo -e "  ${GREEN}2.${NC} 一键部署节点"
    echo -e "  ${GREEN}3.${NC} 查看节点二维码"
    echo -e "  ${RED}0.${NC} 退出"
    read -rp "请选择: " num
